...
int TACGen::visit(const parser::FloorBoxInitStatementASTNodePtr &node)
{
    ...
    if (flr_value->get_is_char()) {
        _floor_inits.insert_or_assign(flr_id->get_value(), HRBox(static_cast<char>(flr_value->get_value())));
    } else {
        _floor_inits.insert_or_assign(flr_id->get_value(), HRBox(static_cast<int>(flr_value->get_value())));
    }
